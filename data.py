...
def print_evaluation_results(test_data, selected_derivs, denotation_correct, example_freq=50, print_output=True):
    """
    Prints output and accuracy. NO NEED TO CALL THIS DIRECTLY -- instead call evaluate in main.py, which
    wraps this.
    :param test_data:
    :param selected_derivs:
    :param denotation_correct:
    :param example_freq: How often to print output
    :param print_output: True if we should print the scores, false otherwise (should never need to set this False)
...
def geoquery_preprocess_lf(lf):
    """
    Geoquery preprocessing adapted from Jia and Liang
    :param lf:
    :return:
    """
    cur_vars = []
